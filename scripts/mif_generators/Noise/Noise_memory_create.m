% Generate memories for the Noise

clear all
close all

%Std of the noise
% In this case, 2 adc for legacy noise
% Multiply by 4 because new electronic will have 12-bit
sigma = 2*4; 
eng_truth = sigma * randn(1,10^8);
eng_truth2 = eng_truth;

%Generating only half of the histogram
eng_truth_half = eng_truth(find(eng_truth > 0));
eng_truth = eng_truth(find(eng_truth >= 0.075)); %Empirichal adjust 
...
%[num, edges] = histcounts(eng_truth, num_table , 'Normalization','probability'); 
...
%Including zero in the first bin of cdf
x_vals = [0,x_vals];
y_vals = [0,y_vals];

samples = length(eng_truth2);


% Don't remember why it is used
norm_inv = norminv([0:0.0005:1],0,2);
norm_inv = norm_inv(1001:end-1);
...
%threshold1 = 0.08;
threshold1 = 0.20; %Change for each STD
...
%threshold2 = 0.08;
threshold2 = 0.13; % Change for each STD
...
sig_uni = randi([0 1],1,length(eng_rand));
pos_values = find(sig_uni == 0);
neg_values = find(sig_uni == 1);

eng_rand(neg_values) = -eng_rand(neg_values);
%eng_rand(pos_values) = -eng_rand(pos_values);
...
%[num2, edges2,bin2] = histcounts(eng_truth2, 150, 'Normalization','probability'); 
%[num, edges,bin] = histcounts(eng_rand, 150, 'Normalization','probability'); 

[num2, edges2,bin2] = histcounts(eng_truth2, 60, 'Normalization','probability'); 
[num, edges,bin] = histcounts(eng_rand, 60, 'Normalization','probability'); 
...
nome_arquivo1 = strcat('../../../HPS_FPGA_Simulador_v1_DE10/Simulador_Arquivos/NOISE_',num2str(sigma),'ADC_PART1.mif');
nome_arquivo2 = strcat('../../../HPS_FPGA_Simulador_v1_DE10/Simulador_Arquivos/NOISE_',num2str(sigma),'ADC_PART2.mif');
nome_arquivo3 = strcat('../../../HPS_FPGA_Simulador_v1_DE10/Simulador_Arquivos/NOISE_',num2str(sigma),'ADC_PART3.mif');
